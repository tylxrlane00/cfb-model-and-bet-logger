# app.py — CFB PPA Monte Carlo Predictor + Supabase + Bet Board fixes (empty-table safe)
...
# Schemas for empty-table safety
BET_LOG_COLS = [
...
def load_table(table: str) -> pd.DataFrame:
    ...
    # Ensure required columns exist (empty-safe)
    if table == "bet_logs":
        for col in BET_LOG_COLS:
            if col not in df.columns:
                df[col] = pd.Series(dtype="object")
    elif table == "saved_projections":
        for col in SAVED_PROJ_COLS:
            if col not in df.columns:
                df[col] = pd.Series(dtype="object")
    return df
...
def load_csv(uploaded):
    # Persist uploaded CSV bytes in session to survive reruns
...
.bet-card { border: 1px solid rgba(255,255,255,.12); border-radius: 12px;
  padding: 10px 12px; margin-bottom: 8px; background: rgba(255,255,255,.04); }
.bet-pick { font-size: 1.05rem; font-weight: 700; }
.bet-type { font-size: .85rem; opacity: .8; }
.bet-odds { font-size: .85rem; opacity: .7; }
.bet-note { font-size: .85rem; opacity: .9; margin-top: 4px; }
...
/* Wider controls in Edit row */
.edit-row .stSelectbox, .edit-row .stButton { min-width: 140px; }
.edit-row .stButton > button { width: 140px; }
</style>
...
sd_home = volatility_sd(home_row, away_row); sd_away = volatility_sd(away_row, home_row)
if not indoor:
...
with c2:
    ...
    st.metric("Spread (Home − Away)", f"{model_spread:+.2f}"); st.metric("Total", f"{model_total:.2f}")
    st.caption("Negative spread ⇒ Home favored.")
with c3:
    st.subheader("Market-Blended Lines")
    st.metric("Blended Spread (H−A)", f"{blend_spread:+.2f}"); st.metric("Blended Total", f"{blend_total:.2f}")
...
with tabs[1]:
            ...
            if dfw.empty:
                ...
                for _, r in dfw.iterrows():
                    ...
                    with st.expander("Edit", expanded=False):
                        with st.container():
                            edit_cols = st.columns([2.4, 1.5, 1.5, 3], gap="small")
    ...
    if submit:
        ...
        if notify:
            msg = (f"**New Bet** by {bettor}\n{home} vs {away}\n"
                   f"{bet_type}: {pick} @ {odds_val if odds_val is not None else '—'} | {stake}u\n"
...
with tabs[2]:
    ...
    if not proj_df.empty:
        ...
        st.download_button(
            ...
            file_name="saved_projections.csv", mime="text/csv",
        )
    else:
        st.caption("No saved projections yet.")

st.caption("Model: z-scored advanced PPA → composite ratings; expected points = baseline + scale × (Off_z − OppDef_z), adjusted by weather/HFA; scores ∼ truncated Normal. Missing data median-imputed; μ/σ guarded.")
