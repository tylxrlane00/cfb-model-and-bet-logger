# app.py — CFB PPA Monte Carlo Predictor (Supabase persistence, summary card, sticky selectors)
...
def load_table(table: str) -> pd.DataFrame:
    ...
    if mode == "supabase":
        try:
            sb = _supabase_client()
            assert sb is not None
            res = sb.table(table).select("*").eq("room", _room()).order("timestamp", desc=True).limit(500).execute()
            if getattr(res, "error", None):
...
def zscore_columns(df: pd.DataFrame, cols: list) -> pd.DataFrame:
    ...
    for c in cols:
        if c in out.columns:
            ...
            out[c + "__z"] = 0.0 if (std is None or std == 0) else (out[c] - mean) / std
...
<style>
/* Sticky top bar */
.sticky-wrap { position: sticky; top: 0; z-index: 999; 
  background: var(--background-color); padding: .5rem 0 .5rem 0;
  border-bottom: 1px solid rgba(255,255,255,.08); }

/* Summary card */
.summary-card {
  border-radius: 14px; padding: 14px 18px; margin: 10px 0 18px 0;
...
.summary-item .label { font-size: .8rem; opacity: .75; }
.summary-item .value { font-weight: 700; font-size: 1.15rem; }
...
with st.sidebar:
    ...
    if df.empty:
        st.info("Upload a CSV (or keep `advancedStats.csv` next to this app).")
    else:
        st.success(f"Loaded {len(df)} rows.")

    st.write("---")
    user_name = st.text_input("Your name (for logs)", value=st.secrets.get("user_name", ""))
...
with t1:
    home = st.selectbox("Home team", team_list, key="home_team_top")
with t2:
    away = st.selectbox("Away team", team_list, index=1 if len(team_list) > 1 else 0, key="away_team_top")
with t3:
    cA, cB = st.columns(2)
    with cA:
        if st.button("Swap"):
            st.session_state["home_team_top"], st.session_state["away_team_top"] = (
                st.session_state["away_team_top"], st.session_state["home_team_top"]
            )
            st.experimental_rerun()
    with cB:
        neutral = st.toggle("Neutral", value=False)
...
# =============================== Tabs ===============================
tabs = st.tabs(["Adjustments", "Bet Logger", "Saved Projections"])
...
model_spread = float(np.mean(margins))   # Home - Away
...
proj_home = float(np.mean(home_scores))
proj_away = float(np.mean(away_scores))
...
<div class="summary-grid">
  <div class="summary-item">
    ...
    <div class="value">{home}: {proj_home:.1f}</div>
  </div>
  <div class="summary-item">
    <div class="label">&nbsp;</div>
    <div class="value">{away}: {proj_away:.1f}</div>
  </div>
  <div class="summary-item">
    <div class="label">Projected Winner</div>
    <div class="value winner">{winner}</div>
  </div>
  <div class="summary-item">
    <div class="label">Blended Spread (H−A)</div>
    <div class="value">{blend_spread:+.2f}</div>
  </div>
  <div class="summary-item">
    <div class="label">Blended Total</div>
    <div class="value">{blend_total:.2f}</div>
  </div>
  <div class="summary-item">
    <div class="label">Recommendation</div>
    <div class="value">{recommendation['confidence']} — {recommendation['label']}</div>
  </div>
</div>
""", unsafe_allow_html=True)
st.markdown('</div>', unsafe_allow_html=True)
...
with c1:
    ...
    st.metric(home, f"{proj_home:.1f}")
    st.metric(away, f"{proj_away:.1f}")
...
# =============================== Bet Logger ===============================
with tabs[1]:
    st.markdown("### Log a Bet (with optional Discord ping)")
    with st.form("bet_form"):
        bettor = st.text_input("Bettor", value=user_name or "")
        ...
        if bet_type == "Spread":
            ...
            line = st.number_input("Line (home-based spread; -3.5 = Home -3.5)", value=market_spread_home, step=0.5, format="%.1f")
            odds = st.number_input("Price (American)", value=-110, step=5)
            pick = f"{side} {line:+g}"
        elif bet_type == "Total":
            ou = st.selectbox("Over / Under", ["Over", "Under"])
            line = st.number_input("Total", value=market_total, step=0.5, format="%.1f")
            odds = st.number_input("Price (American)", value=-110, step=5)
            pick = f"{ou} {line:g}"
        else:
            side = st.selectbox("Side", [f"Home ({home})", f"Away ({away})"])
            odds = st.number_input("Price (American)", value=-110, step=5)
            pick = f"{side} ML {odds:+d}"

        stake = st.number_input("Stake (units)", value=1.0, step=0.5, min_value=0.0)
    ...
    if submit:
        ...
        row = {
            ...
            "blended_spread": round(blend_spread, 2),
            "blended_total": round(blend_total, 2),
        ...
        if notify:
            msg = (f"**New Bet Logged** by {bettor or 'Unknown'}\n"
                   f"{home} vs {away}\n"
                   f"{bet_type}: {pick} @ {int(odds):+d} | {stake}u\n"
                   f"Model: spread {model_spread:+.2f}, total {model_total:.2f} | "
                   f"Blend: spread {blend_spread:+.2f}, total {blend_total:.2f}\n"
                   f"Reco: {recommendation['confidence']} — {recommendation['label']} (EV {recommendation['ev']:.3f}/1u)")
    ...
    if not bet_df.empty:
        st.dataframe(bet_df.head(50), width='stretch')
    else:
...
with tabs[2]:
    ...
    if not proj_df.empty:
        st.dataframe(proj_df.head(50), width='stretch')
        st.download_button(
            "⬇️ Download saved projections (CSV)",
            data=proj_df.to_csv(index=False).encode("utf-8"),
            file_name="saved_projections.csv",
            mime="text/csv",
        )
    else:
...
# Footer
st.caption("Model: z-scored advanced PPA → composite ratings; expected points = baseline + scale × (Off_z − OppDef_z), adjusted by weather/HFA; scores ∼ truncated Normal. Missing data median-imputed; µ/σ guarded.")
