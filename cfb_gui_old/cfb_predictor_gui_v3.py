# app.py
# Streamlit GUI for CFB predictor (ADVANCED SCHEMA ONLY, PPA-based)
# Uses weekly CSV with columns:
# team, offense.ppa, offense.successRate, offense.explosiveness, offense.powerSuccess,
# offense.pointsPerOpportunity, offense.fieldPosition.averageStart, offense.standardDowns.successRate,
# offense.passingDowns.ppa, offense.rushingPlays.ppa, offense.passingPlays.ppa,
# defense.ppa, defense.successRate, defense.explosiveness, defense.powerSuccess, defense.stuffRate,
# defense.pointsPerOpportunity, defense.fieldPosition.averageStart, defense.havoc.total,
# defense.rushingPlays.ppa, defense.passingPlays.ppa
...
# Turnover estimation (very gentle)
TO_TAKE_K   = 0.15   # how much defensive havoc z bumps takeaways (~per game)
TO_GIVE_K   = 0.10   # how much offensive success z reduces giveaways
TO_PRESSURE = 0.10   # how much opponent's havoc z increases giveaways
TO_CLAMP    = 2.5    # clamp absolute net TO to avoid extremes

# Market blend defaults
MARKET_BLEND = {"use": True, "spread_weight": 0.25, "total_weight": 0.25}

# Advanced index weights (offense/defense); these are z-score weights
ADV_OFF_WEIGHTS = {
    "ppa": 0.45,
    "success": 0.25,
    "explosive": 0.15,
...
ADV_DEF_WEIGHTS = {
    "ppa": 0.50,          # lower is better (we invert)
    ...
    "havoc": 0.03,        # higher is better (positive)
    "stuff": 0.00,        # optional small positive if you like (left at 0 by default)
}

# Situational multipliers (all bounded)
# - power success mismatch (short-yardage)
POWER_MULT_CAP = (0.90, 1.10)
POWER_MULT_COEF = 0.05
# - points per opportunity mismatch (finishing drives/red zone-ish)
PPO_MULT_CAP = (0.85, 1.15)
PPO_MULT_EXP = 0.50   # exponent on defensive allowed term (same spirit as before)
# - rush/pass mismatch
RPP_MULT_CAP = (0.90, 1.10)
RPP_MULT_COEF = 0.04
# - field position mild nudge
FP_MULT_CAP = (0.97, 1.03)
FP_MULT_COEF = 0.01
# - tempo proxy from success/explosive
TEMPO_SUCCESS_COEF = 0.06
TEMPO_EXPLOSIVE_COEF = -0.02

# League baselines for a few metrics if you ever want to override dynamic ones
LEAGUE_STATIC = {
...
def load_df(src) -> pd.DataFrame:
    ...
    # Ensure all expected columns exist (missing ones will be filled with NaN)
    for c in ADV_COLS:
...
def compute_baselines(df: pd.DataFrame) -> Dict[str, Dict[str, float]]:
    """Compute mean/std dynamically from the uploaded CSV (robust each week)."""
    bl = {}
    cols_for_stats = [c for c in df.columns if c != "team"]
    for c in cols_for_stats:
        series = pd.to_numeric(df[c], errors="coerce")
        m = float(series.mean(skipna=True)) if series.notna().any() else 0.0
        s = float(series.std(ddof=0, skipna=True)) if series.notna().any() else 1.0
        if s < 1e-9: s = 1.0
        bl[c] = {"mean": m, "std": s}
    # Provide PPO/FP defaults if missing/odd
...
def defense_index(t: Team, bl) -> float:
    """Higher = better defense (i.e., suppresses opposing offense)."""
    return (
...
def tempo_proxy(team: Team, bl) -> float:
    """Gentle tempo proxy from success/explosiveness; bounded to reasonable range."""
    base = AVG_TEMPO
...
def combined_plays(home: Team, away: Team, wx: Weather, bl) -> float:
    """Use tempo proxies + small weather friction for tempo."""
...
def situational_ppd_multiplier(off: Team, opp_def: Team, bl) -> float:
    # Power success (short-yardage)
    pow_off = zcol(off.off_power, bl, "offense.powerSuccess")
    pow_def = zcol(opp_def.def_power, bl, "defense.powerSuccess")
    m_power = 1.0 + POWER_MULT_COEF * (pow_off - (-pow_def))  # def lower is better; subtracting negative penalizes good Ds
    m_power = clamp(m_power, *POWER_MULT_CAP)

    # Points per opportunity (finishing drives)
    ...
    # Field position nudge
    fp_off = zcol(off.off_fp_start, bl, "offense.fieldPosition.averageStart")
    fp_def = -zcol(opp_def.def_fp_start, bl, "defense.fieldPosition.averageStart")  # lower allowed FP is better D
...
def expected_net_turnovers(home: Team, away: Team, bl) -> float:
    """Estimate net TO (home advantaged) from havoc + success.
       Very gentle so it won't dominate outcomes."""
...
def predict_game(home: Team, away: Team, wx: Weather, bl, vegas_spread=None, vegas_total=None,
    ...
    # Indices
    home_off = offense_index(home, bl); home_def = defense_index(home, bl)
    away_off = offense_index(away, bl); away_def = defense_index(away, bl)

    # Drives/plays
    plays = combined_plays(home, away, wx, bl)
    total_drives = max(10.0, plays / PLAYS_PER_DRIVE)
    den = max(1e-6, tempo_proxy(home, bl) + tempo_proxy(away, bl))
    drives_home = total_drives * (tempo_proxy(home, bl) / den)
    ...
    home_pts_adj = max(0.0, home_pts_adj); away_pts_adj = max(0.0, away_pts_adj)

    return {
        "home_team": home.name, "away_team": away.name,
        "home_points": round(home_pts_adj,1), "away_points": round(away_pts_adj,1),
        "spread_home_minus_away": round(final_spread,1), "total_points": round(final_total,1),
        "model_spread_no_market": round(model_spread,1), "model_total_no_market": round(model_total,1),
        ...
        "home_ppd": round(home_ppd,3), "away_ppd": round(away_ppd,3),
        "drives_home": round(drives_home,1), "drives_away": round(drives_away,1),
        "net_turnovers_home_adv": round(net_to,2),
        "wx_indoor": wx.is_indoor, "wx_temp_f": wx.temp_f, "wx_wind_mph": wx.wind_mph, "wx_precip": wx.precip
    }
...
def tier_from_ev(ev_pct: float) -> str:
    if ev_pct >= EV_THRESHOLDS["strong"]: return "STRONG"
    if ev_pct >= EV_THRESHOLDS["lean"]:   return "LEAN"
    return "PASS"

def spread_signal(model_spread: float, line_home_minus_away: float, odds: float = DEFAULT_BOOK_ODDS):
    edge = model_spread - line_home_minus_away
    ...
    return {"market_line": line_home_minus_away, "model_spread": round(model_spread,2),
            "edge_pts": round(edge,2), "p_home_covers": round(p_home_covers,3),
            "odds": odds, "break_even": round(be,3), "ev_pct": round(ev,3), "side": side, "signal": sig}
...
def total_signal(model_total: float, line_total: float, odds: float = DEFAULT_BOOK_ODDS):
    ...
    sig  = tier_from_ev(abs(ev)) if ev > 0 else "PASS"
    return {"market_total": line_total, "model_total": round(model_total,2),
            "edge_pts": round(edge,2), "p_over": round(p_over,3),
            "odds": odds, "break_even": round(be,3), "ev_pct": round(ev,3), "side": side, "signal": sig}

def moneyline_signal(model_home_win_prob: float, odds_home: float = None, odds_away: float = None):
    out = {}
    if odds_home is not None:
        be_h = break_even_prob(odds_home); ev_h = expected_value_pct(model_home_win_prob, odds_home)
        out["HOME"] = {"model_p": round(model_home_win_prob,3), "odds": odds_home,
                       "break_even": round(be_h,3), "ev_pct": round(ev_h,3),
                       "signal": tier_from_ev(abs(ev_h)) if ev_h > 0 else "PASS"}
    if odds_away is not None:
        p_away = 1.0 - model_home_win_prob
        be_a = break_even_prob(odds_away); ev_a = expected_value_pct(p_away, odds_away)
        out["AWAY"] = {"model_p": round(p_away,3), "odds": odds_away,
                       "break_even": round(be_a,3), "ev_pct": round(ev_a,3),
...
with st.sidebar:
    ...
    st.info("Schema detected: **advanced (PPA)**")
...
# Helpers to parse Vegas inputs
def parse_spread(raw: str, home_name: str, away_name: str) -> Optional[float]:
    raw = raw.strip()
    if not raw: return None
    rl = raw.lower()
    if rl in {"pk","pick","pickem","pick'em","0","0.0"}: return 0.0
    ...
    if not (is_home or is_away):  # raw Home−Away number
        return num

    # Express as Home−Away spread: positive means HOME favored
    if is_home:
        return (+abs(num) if num < 0 else -abs(num)) if ("+" in rl or "-" in rl) else num
    else:
        return (-abs(num) if num < 0 else +abs(num)) if ("+" in rl or "-" in rl) else -num
...
if run:
    try:
        # Baselines from the uploaded CSV
        ...
        oh = parse_float(odds_home); oa = parse_float(odds_away)
        ...
        for _ in range(n_sims):
            ...
            # add gentle noise from net TO & HFA (already embedded in PPD/spread, so keep tiny)
            hp += np.random.normal(0, 0.15) + 0.15 * pred["hfa_points"]
            ap += np.random.normal(0, 0.15)
            home_scores.append(max(0, hp)); away_scores.append(max(0, ap))
        ...
        with st.expander("Diagnostics"):
            ...
            # quick snapshots of a few advanced mismatches
            def mk_row(name, off_v, def_v):
            ...
            fig1 = plt.figure(); plt.hist(home_scores - away_scores, bins=40); plt.title("Simulated Margin (Home - Away)"); st.pyplot(fig1)
            fig2 = plt.figure(); plt.hist(home_scores + away_scores, bins=40); plt.title("Simulated Total Points"); st.pyplot(fig2)

        # Nice Vegas sentence for clarity
        def vegas_line_text(spread_val: float, home_label: str, away_label: str) -> str:
            if spread_val is None: return "n/a"
            if spread_val > 0:  return f"{home_label} -{abs(spread_val):.1f} (i.e., {away_label} +{abs(spread_val):.1f})"
            if spread_val < 0:  return f"{away_label} -{abs(spread_val):.1f} (i.e., {home_label} +{abs(spread_val):.1f})"
            return "Pick'em"

        if sig_spread:
            st.subheader("Spread Signal")
            st.write(f"**Vegas says:** {vegas_line_text(vs_spread, home.name, away.name)}")
            model_line_txt = f"{home.name} -{abs(sig_spread['model_spread']):.1f}" if sig_spread["model_spread"]>0 else f"{away.name} -{abs(sig_spread['model_spread']):.1f}"
            st.write(f"**Model says:** {model_line_txt}")
            p_cover = sig_spread["p_home_covers"] if sig_spread["side"]=="HOME" else (1.0 - sig_spread["p_home_covers"])
        ...
        out = pred.copy()
        if sig_spread:
            out.update({"sig_spread_line": sig_spread["market_line"], "sig_spread_model": sig_spread["model_spread"],
                        "sig_spread_edge_pts": sig_spread["edge_pts"], "sig_spread_p_home_covers": sig_spread["p_home_covers"],
                        "sig_spread_ev_pct": sig_spread["ev_pct"], "sig_spread_side": sig_spread["side"],
                        "sig_spread_reco": sig_spread["signal"]})
        if sig_total:
            out.update({"sig_total_line": sig_total["market_total"], "sig_total_model": sig_total["model_total"],
                        "sig_total_edge_pts": sig_total["edge_pts"], "sig_total_p_over": sig_total["p_over"],
                        "sig_total_ev_pct": sig_total["ev_pct"], "sig_total_side": sig_total["side"],
